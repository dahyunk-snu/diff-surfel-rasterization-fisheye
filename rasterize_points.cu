	...
	const bool debug)
{
  ...
  torch::Tensor contrib_sum = torch::full({P, 1}, 0.0, float_opts);
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		contrib_sum.contiguous().data<float>(),
		radii.contiguous().data<int>(),
		debug);
  }
  return std::make_tuple(rendered, out_color, out_others, radii, contrib_sum, geomBuffer, binningBuffer, imgBuffer);
}
